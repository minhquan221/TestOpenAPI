function makeFreeze() {
    $('.table-freeze-multi').each(function () {
        ...
        //Duc update
        clone.find('input').removeAttr('id').removeAttr('name');


        var colsNumber = table.attr('data-colsNumber') || table.find('tbody tr:first th').length;
        ...
        //sizing
        var scrollHeight = table.attr('data-scrollHeight') || wrapper.parent().closest('*').height();
        ...
        leftblock.bind('DOMMouseScroll', function (e) {
            console.log('x');
            e.preventDefault();
            return false;
        });
